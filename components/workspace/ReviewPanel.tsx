...
export default function ReviewPanel({ record, rubric, engines, onChange }: Props) {
  ...
  return <div className="review-layout">
    <section className="panel human-review"><div className="panel-heading"><div><span className="eyebrow">CRITERIO DEL PROFESOR</span><h3>De la escucha a la evidencia.</h3></div><ClipboardCheck size={22} /></div><p className="muted text-small">Confirma cada herramienta. Una detección pendiente mantiene la nota abierta.</p>
      <div className="effect-list">{EFFECTS.map(effect => <div className="effect-row" key={effect.id}>
        <div className="effect-heading"><h4>{effect.label}{!required.has(effect.id) && <small>opcional</small>}{marksFor(record, effect.id).length > 0 && <small className="mark-count">{marksFor(record, effect.id).length} marca(s)</small>}</h4><div className="segmented" role="group" aria-label={`Revisión de ${effect.label}`}>{labelOptions.map(option => <button key={option.value} aria-pressed={record.labels[effect.id] === option.value} className={record.labels[effect.id] === option.value ? `selected ${option.value}` : ''} onClick={() => onChange({ labels: { ...record.labels, [effect.id]: option.value } })}>{option.value === 'present' && <Check size={12} />}{option.label}</button>)}</div></div>
        <p>{effect.hint}</p><label className="evidence-input"><span>Evidencia</span><input aria-label={`Evidencia de ${effect.label}`} placeholder="Ej.: 0:12–0:18 · comparación con la fuente" value={record.evidence[effect.id]} onChange={e => onChange({ evidence: { ...record.evidence, [effect.id]: e.target.value } })} /></label>
      </div>)}</div>
    ...
    <aside className="score-panel"><span className="eyebrow">EVALUACIÓN / {score.maxTotal.toLocaleString('es')} PUNTOS</span><div className="score-display" data-testid="final-score">{score.final === null ? <span className="pending-score">En revisión</span> : <><strong>{score.final.toLocaleString('es', { maximumFractionDigits: 2 })}</strong><span>/ {score.maxTotal.toLocaleString('es')}</span></>}</div><p>{score.manual ? 'Nota ajustada por el profesor.' : score.pending.length ? `${score.pending.length} decisiones pendientes. Rango posible: ${score.minimum}–${score.maximum}.` : 'Revisión completa. Calculada con la rúbrica.'}</p>
      ...
      {record.student && <div className={`publish-state ${record.published ? 'on' : ''}`}>{record.published ? `Nota y feedback publicados${record.publishedGrade ? ` (${record.publishedGrade.final.toLocaleString('es', { maximumFractionDigits: 2 })} / ${record.publishedGrade.maxTotal})` : ''}: el estudiante ve esa nota aunque cambies la rúbrica o la revisión.` : 'Nota sin publicar: el estudiante solo ve las mediciones.'}</div>}
