...
export default function ModelAdvice({ record, analyzed, model, engines, teacher, onChange, onEngines }: Props) {
  ...
  return <div className="model-advice">
    ...
    {external ? <div className="external-advice">
      <div className="engine-summary-row"><span className="eyebrow"><Sparkles size={14} /> {modelInfo?.label ?? engines.models[external]}</span><span className="mono">{runs} ejecución{runs > 1 ? 'es' : ''} · ≈ {cost.toFixed(3)} $</span>{teacher && onEngines && <button className="text-button" onClick={onEngines}>Cambiar motor</button>}</div>
      {teacher && <p className="notice">Se envían el audio preparado o las imágenes, las métricas, el nombre del archivo, la sinopsis y el contexto al proveedor. La clave es la guardada en «Motores de IA».</p>}
    ...
    {ai && <div className="ai-response"><span className="eyebrow">{teacher ? 'SUGERENCIA EXTERNA' : 'LECTURA ORIENTATIVA'} · {ai.meta.modelLabel}</span>
      ...
      <small>{ai.meta.runs} ejecución(es) · {ai.meta.agreement === null ? 'Sin contraste entre ejecuciones' : `${Math.round(ai.meta.agreement * 100)} % de acuerdo`} · coste orientativo ${ai.meta.estimatedCostUsd.toFixed(4)}</small></div>}
  </div>;
}
