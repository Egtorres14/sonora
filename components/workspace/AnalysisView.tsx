...
import { ArrowLeft, Download, FileAudio, Printer, UserRound } from 'lucide-react';
...
interface Props { record: ReviewRecord; analyzed: AnalyzedAudio | null; audioUrl: string; records: ReviewRecord[]; model: LocalModel | null; rubric: RubricConfig; engines: EngineSettings; teacher: boolean; onChange: (patch: Partial<ReviewRecord>) => void; onBack: () => void; onEngines?: () => void; referenceUrl: string; onReference: (id: string) => void; referenceId: string }
const fmtDate = (iso: string) => new Date(iso).toLocaleString('es', { dateStyle: 'long', timeStyle: 'short' });

export default function AnalysisView({ record, analyzed, audioUrl, records, model, rubric, engines, teacher, onChange, onBack, onEngines, referenceUrl, onReference, referenceId }: Props) {
  ...
  const graded = score.final !== null;
  const evidence = useMemo(() => buildEvidence(record, rubric), [record, rubric]);
  ...
  return <div className="analysis-view">
    <div className="file-heading"><div><button className="text-button back-button" onClick={onBack}><ArrowLeft size={14} /> {teacher ? 'Biblioteca' : 'Mis entregas'}</button><h2><FileAudio size={25} />{record.name}</h2><p>{record.origin === 'synthetic' ? 'DEMO SINTÉTICA' : record.student ? 'ENTREGA DE ESTUDIANTE' : 'MUESTRA REAL'} <span>·</span> {record.features.format.sampleRate.toLocaleString('es')} Hz <span>·</span> {record.features.format.bitDepth || '?'} bit <span>·</span> {record.features.format.channels} canales <span>·</span> {record.features.format.duration.toFixed(1)} s</p>{record.student && <p className="student-line"><UserRound size={13} /> {teacher ? `Entregado por ${record.student.name}` : 'Tu entrega'} <span>·</span> {fmtDate(record.student.submittedAt)}</p>}</div><div className="toolbar">{teacher && <button className="button secondary" onClick={() => downloadFile(JSON.stringify({ review: record, score, evidencias: evidence }, jsonSafe, 2), `${record.name}.informe.json`)}><Download size={15} /> Informe JSON</button>}<button className="icon-button" aria-label="Imprimir o guardar informe como PDF" onClick={() => window.print()}><Printer size={18} /></button></div></div>
    ...
    <section className="panel signal-panel"><div className="panel-heading"><div><span className="eyebrow">EXPLORADOR DE SEÑAL</span><h3>Escucha dónde ocurre.</h3></div><div className="segmented" aria-label="Vista de audio"><button aria-pressed={tab === 'wave'} className={tab === 'wave' ? 'selected' : ''} onClick={() => setTab('wave')}>Forma de onda</button><button disabled={!analyzed} aria-pressed={tab === 'spectrum'} className={tab === 'spectrum' ? 'selected' : ''} onClick={() => setTab('spectrum')}>Espectrograma</button></div></div>
      ...
      {audioUrl ? <AudioPlayer src={audioUrl} features={record.features} extraMarkers={extraMarkers} seekRef={seekRef} /> : <div className="empty-audio">Este registro contiene métricas y etiquetas. Vuelve a subir el audio original para escucharlo; se reconocerá por su huella.</div>}
    ...
    <EvidencePanel items={evidence} onSeek={seek} canSeek={!!audioUrl} teacher={teacher} />
    ...
    {teacher ? <>
      ...
      <ReviewPanel record={record} rubric={rubric} onChange={onChange} />
      ...
      <section className={`panel student-status ${graded ? 'graded' : ''}`}><div className="panel-heading"><div><span className="eyebrow">ESTADO DE TU ENTREGA</span><h3>{graded ? 'Revisión terminada.' : 'Pendiente de revisión.'}</h3></div>{graded && <div className="score-display" data-testid="student-score"><strong>{score.final!.toLocaleString('es', { maximumFractionDigits: 2 })}</strong><span>/ {score.maxTotal}</span></div>}</div>
        {graded ? <><div className="score-breakdown">{[{ label: 'Formal', value: score.formal.total, max: score.formalMax }, { label: 'Técnica', value: score.technical.total, max: score.technicalMax }, { label: 'Creatividad', value: score.creative.total, max: score.creative.max }].map(row => <div key={row.label}><span>{row.label}</span><b>{row.value}<small> / {row.max}</small></b><div><i style={{ width: `${row.max ? row.value / row.max * 100 : 0}%` }} /></div></div>)}{score.bonus > 0 && <div className="bonus-row"><span>Extra</span><b>+{score.bonus}</b></div>}</div>{record.notes ? <div className="field"><span>Comentarios del profesor</span><p className="context-text">{record.notes}</p></div> : <p className="muted text-small">El profesor no ha dejado comentarios.</p>}</>
          : <p className="muted text-small">Tu profesor aún no ha completado la revisión. Mientras tanto puedes escuchar tu archivo, ver sus mediciones y completar la sinopsis.</p>}
      </section>
    </>}
  </div>;
}
