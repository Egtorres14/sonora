...
export default function App() {
  ...
  const modelStale = w.modelStale;
  // Volver a pulsar «Laboratorio» con un archivo abierto devuelve a la zona de carga, en lugar de no hacer nada.
  const goTo = (id: WorkspaceView) => (id === 'lab' && w.view === 'lab' && w.selected ? w.clearSelection() : w.setView(id));
  return <div className="app-shell"><a className="skip-link" href="#main-content">Saltar al contenido</a><aside className="sidebar"><a href="#" className="brand" onClick={e => { e.preventDefault(); w.setView('lab'); }} aria-label="Sonora, laboratorio de audio"><BrandBars /><span>sonora<span className="brand-dot">.</span><small>LABORATORIO DE AUDIO</small></span></a><div className="sidebar-section-label">{teacher ? 'ESPACIO DEL PROFESOR' : 'ESPACIO DEL ESTUDIANTE'}</div><nav aria-label="Navegación principal">{nav.map(item => <button key={item.id} aria-current={w.view === item.id ? 'page' : undefined} className={w.view === item.id ? 'active' : ''} onClick={() => goTo(item.id as WorkspaceView)}><item.icon size={19} strokeWidth={1.5} /><span>{item.label}</span>{item.id === 'library' && w.records.length ? <b>{w.records.length}</b> : <small>{item.number}</small>}</button>)}</nav>
    ...
    <div className="workspace-shell"><header className="workspace-header"><div className="breadcrumb"><span>{teacher ? 'Profesor' : 'Estudiante'}</span><ChevronRight size={13} /><b>{current.label}</b></div><div className="header-right"><span className="local-badge"><span className="status-dot" />{teacher ? 'Motor local activo' : studentName}</span><span className="header-divider" /><span className="profile-avatar" title={teacher ? 'Profesor' : studentName}>{teacher ? 'P' : studentName.trim().charAt(0).toUpperCase() || 'E'}</span><button className="text-button" onClick={w.leave} aria-label="Salir y volver al menú de entrada"><LogOut size={14} /> Salir</button></div></header><main id="main-content" className="workspace-content">
      ...
      {w.view === 'lab' && (w.selected ? <AnalysisView record={w.selected} analyzed={w.analyzed} audioUrl={w.audioUrl} records={w.records} model={modelStale ? null : w.model} rubric={w.rubric} engines={w.engines} teacher={teacher} onChange={patch => w.change(w.selected!.id, patch)} onBack={() => w.setView('library')} onNewUpload={w.clearSelection} onEngines={() => w.setView('engines')} referenceId={w.referenceId} referenceUrl={w.referenceUrl} onReference={w.reference} /> : <UploadArea busy={w.busy} onFiles={w.files} onDemo={w.demo} student={teacher ? undefined : studentName} nameFormat={w.rubric.formal.fileNamePattern} />)}
      ...
      {w.view === 'learning' && teacher && <Suspense fallback={<div className="empty-table">Cargando laboratorio de aprendizaje…</div>}><LearningView records={w.allRecords} model={w.model} history={w.modelHistory} stale={modelStale} audioIds={w.audioIds} reanalyzing={w.busy} onReanalyze={w.reanalyze} holdout={w.holdout} onFreeze={w.freezeHoldout} onUnfreeze={w.unfreezeHoldout} onModel={w.saveModel} onLibrary={() => w.setView('library')} /></Suspense>}
