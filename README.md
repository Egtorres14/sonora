...
- **Profesor**: entra con el PIN acordado (`Felipebolano2026`; en el código solo va su hash, pero el repositorio es público, así que cámbialo si necesitas privacidad real) y ve la biblioteca completa con el nombre de cada estudiante, califica, elimina, importa/exporta, entrena el modelo y **edita la rúbrica** (puntos, frecuencia exigida, penalizaciones, herramientas obligatorias, duración, extra y el **formato exigido del nombre de archivo**, que se comprueba contra el nombre del estudiante y se le muestra antes de entregar).
...
- **Revisión humana:** presencia, ausencia o pendiente por herramienta; evidencia escrita; sinopsis y contexto; nota derivada de la rúbrica y ajuste manual independiente. Al publicar, la nota queda **congelada** para el estudiante: cambiar la rúbrica, retocar etiquetas o reanalizar el audio no la mueve; el profesor ve la diferencia y decide si vuelve a publicar. Los comentarios no borran una nota manual y una etiqueta pendiente no se convierte en ausencia.
- **Biblioteca:** audio original, métricas y anotaciones en IndexedDB; deduplicación SHA-256, búsqueda, filtros, páginas de 20/50/100 muestras, exportación JSON/CSV, importación validada y eliminación individual. El CSV incluye notas finales y su procedencia manual/calculada/pendiente.
- **Modelo local:** bosques aleatorios por herramienta, entrenamiento en worker y validación con tres particiones separadas por grabación de origen. Se guardan las matrices de confusión y el modelo. Las sugerencias se abstienen con resultados insuficientes y no cambian etiquetas ni notas. Cada entrenamiento deja un resumen en el **historial**, con la diferencia respecto al anterior por herramienta. Con un **conjunto de evaluación congelado** (una quinta parte de los orígenes, apartados una vez y fuera del entrenamiento), cada modelo se mide además sobre el mismo examen: esa es la cifra comparable entre entrenamientos. El modelo caduca solo cuando cambia algo que aprende (mediciones, etiquetas, grupo de origen o procedencia); escribir el feedback de un estudiante no lo invalida. Los registros medidos por una versión anterior de las características siguen puntuando, quedan fuera del entrenamiento con un contador visible y se reanalizan solos al abrirlos si tienen audio guardado (o por lotes desde Modelo local).
