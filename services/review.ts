...
export const calculateReview = (record: ReviewRecord, r: RubricConfig = DEFAULT_RUBRIC) => {
  ...
  const minimumCreative = Math.max(0, creativeTotal - unknownTools * r.creative.missingToolPenalty - (record.overprocessing === 'unknown' ? r.creative.overprocessingPenalty.Severo : 0));
